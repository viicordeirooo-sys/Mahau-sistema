...
- **Seeding:** `seedCombos()` (button on the Fichas page) generates the combo recipe `fichas`. Products are created manually (`+ Novo Produto` / Editar em Massa). The old product seed and the legacy **ZigPay** POS import (`parseZigFile`/`importZigPay`) were both removed in the events refactor — not to be confused with the current per-event Zig import (Sprint 3, `importVendidosHTML`/`aplicarBaixaEvento`), which only baixa stock and never touches revenue.
