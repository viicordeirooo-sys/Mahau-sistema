...
- **Dependencies** are loaded from CDNs in the `<head>` (lines 7-9): Firebase 10.12.0 *compat* SDK (`firebase-app-compat`, `firebase-auth-compat`, `firebase-firestore-compat`) and SheetJS `xlsx` 0.18.5. The app itself has no local `node_modules` (Firebase CDN compat SDKs are loaded directly in `index.html`). However, there IS a local `node_modules` used exclusively by `set-claims.mjs` (Firebase Admin SDK), which is a local-only admin tool — never deployed.
- **Persistence** is Firebase Firestore. The config is hardcoded at `index.html:253-260` (project `mahau-sistema`). **Authentication:** Firebase Auth (email/senha) — the whole app sits behind a login screen (`doLogin`/`onAuthStateChanged` → `showApp`/`showLogin`, `bindData`/`unbindData` attach/detach the listeners; auth block at `index.html:2996-3105`). Access control uses **RBAC** with 3 roles set via custom claims in the JWT token:
  - **admin:** full access (RWD on all collections)
  - **manager:** read everything, write on operational collections, no deletes, no config writes
  - **staff:** read+write on counting collections (`estoques`, `vendas_zig`), read-only on catalog (`produtos`, `mapeamento_zig`, `semanas_conciliacao`), no access to admin areas (`fichas`, `compras`, `eventos`, `custos`, `inventarios`, `cortesias_semana`, `estornos_semana`, `conciliacoes`, `config`, `lixeira`)

  Claims are set locally via `set-claims.mjs` (Firebase Admin SDK script, requires service account key + `roles.json` with email→role mapping). Users must log out and back in for the claim to take effect in their token. See full permission tier table in `firestore.rules` header. Firestore rules implement RBAC by reading the `role` custom claim from the JWT. Helpers: `isAdmin()`, `isManager()`, `isStaff()`. Rules are organized by tier (A: operational restricted, B: shared catalog, C: counting/operational write) with deny-by-default. Rules are applied manually via the Firebase console (not auto-deployed); see `firestore.rules` header for full version history. Per-org `orgId` isolation is a future phase. If a write fails, the app surfaces the error via toast.
...
Conciliação semanal que cruza dados de venda da Zig com a contagem física, **separado** do evento financeiro `eventos`. Página **"🔄 Conciliação"** (`renderConciliacao`) com 6 sub-abas (`S.concilTab`): Semanas · Estoque · Saída Zig · Cortesias · Estornos · Resultado. Uma semana ativa (`S.concilSemanaSel`) é o contexto dos imports; usa o período da semana, **não** o filtro de mês global. O **Mapeamento Zig→Estoque** (antes uma aba aqui) virou **página própria de cadastro central** — id `"mapa"`, "🔗 Mapeamento Zig" no menu lateral junto de Produtos (`renderMapa`, container `#p-mapa`); reusa `concilMapaHTML`/`mapaFormHTML` e re-renderiza via `rerenderMapa` (`S.page==="mapa"?renderMapa():renderConciliacao()`).
